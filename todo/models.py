...
class User(models.Model):
    ...
    login_token = models.CharField(max_length = LOGIN_TOKEN_MAX_LENGTH, unique=True, null=True)
...
class Task(models.Model):
    ...
    MAX_STATUS_LENGTH = 7
    STATUS_CHOICES = [
        ('OPEN', 'Open'), # here, first element is the value that will be stored in the DataBase 
        ('WORKING', 'Working'), # second element is the value that is for frontEnd(human readable)
        ('DONE', 'Done'),
        ('OVERDUE', 'Overdue'),
    ]
    ...
    description = models.CharField(max_length = MAX_DESCRIPTION_LENGTH, null = False, blank = False)
